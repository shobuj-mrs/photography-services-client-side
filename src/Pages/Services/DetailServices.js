...
import AddRevieewDisplay from './AddRevieewDisplay';
...
const DetailServices = () => {
    ...
    const handleAddToReview = e => {
        ...
        fetch('https://assignment-11-service-server-side.vercel.app/reviews', {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        fetch(`https://assignment-11-service-server-side.vercel.app/reviews/${_id}`)
            .then(res => res.json())
    ...
    return (
        <div className='my-12 mx-12 grid grid-cols-1 lg:grid-cols-2 gap-2'>
            ...
            <div>
                ...
                <div>
                    {
                        addedReview.map(rvw => <AddRevieewDisplay key={rvw._id} rvw={rvw}></AddRevieewDisplay>)
                    }
                </div>
            </div>
